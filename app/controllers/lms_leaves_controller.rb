class LmsLeavesController < ApplicationController
  ...
  before_filter :find_current_year_settings, :only => [:new, :create, :proces, :deduct, :update]
  ...
  def create
    ...
    @leave.safe_attributes = lms_leave_params
    if @leave.valid?
      monthly_history = LmsMonthlyLeaveHistory.where(:user_id => @current_month_leave_history.user_id,
                                                     :lms_yearly_leave_history_id => @current_month_leave_history.lms_yearly_leave_history_id,
                                                     :month => params[:lms_leave][:from_date].to_date.month).first
  ...
  def deduct
    ...
    if leave.lms_leave_status.approved?
      if LmsLeaveCategory::WORK_FROM_HOME[leave.leave_category_id]
        ...
        attributes = params[:lms_leave_status].merge(:status => LmsLeaveStatus::PROCESSED, :processed_by => @employee.id, :processed_on => Time.now)
        unless leave.lms_leave_status.update_attributes(update_lms_leave_params(attributes))
          @leave = leave
          @applier = @leave.employee
