class LmsLeave < ActiveRecord::Base
  ...
  has_many :split_leaves, ->{ where("lms_leaves.id <> parent_leave_id")}, :class_name => 'LmsLeave', :foreign_key => 'parent_leave_id'
  ...
  scope :not_processed, -> {where("status <> ?", LmsLeaveStatus::PROCESSED).joins(:lms_leave_status)}
  ...
  def validate_leave_dates_object
    ...
    exist_leav_recs = employee.current_year_leave_history.lms_leaves.joins(:lms_leave_status)
    .where("(parent_leave_id IS NULL OR lms_leaves.id = parent_leave_id) AND (status IN (?))", [LmsLeaveStatus::PENDING, LmsLeaveStatus::APPROVED, LmsLeaveStatus::PROCESSED])
  ...
  def self.fetch_leaves(status)
    ...
    self.where(condition).joins(:employee, :lms_leave_status, :lms_yearly_leave_history => :lms_yearly_setting)
