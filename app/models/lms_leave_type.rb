class LmsLeaveType < ActiveRecord::Base
  ...
  has_and_belongs_to_many :yearly_settings, :class_name => "LmsYearlySetting", :join_table => 'lms_leave_types_lms_yearly_settings'
  ...
  validates_format_of :identifier, :with => /\A[a-z]+_*[a-z]+\Z/, :if => Proc.new { |t| t.identifier_changed? }
  ...
  def update_present_year_leave_history
    if days_changed?
      ...
      if year_settings
        if year_settings.leave_types.where(["lms_leave_type_id = ?", self.id]).first
